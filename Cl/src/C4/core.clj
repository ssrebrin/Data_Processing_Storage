...
(defn request-resources-msg
  ...
  [factory-state ware needed-amount storage-atom]
  (let [available (min needed-amount @storage-atom)]
    (when (> available 0)
      (swap! storage-atom - available))
    available))
...
(defn notify-msg
  ...
  (let [{:keys [bill buffer]} state]
    ...
    (if-not (contains? bill ware)
      ...
      (let [
            missing  (max 0 (- (bill ware) (get buffer ware 0)))
            taken    (if (pos? missing) (let [res (atom 0)]
                         (try (swap! storage-atom
                                  (fn [cur] (let [t (min missing cur)]
                                      (reset! res t) (- cur t))))
                           (catch IllegalStateException _
                             (reset! res 0))) @res) 0)
            buffer   (if (pos? taken)
                       (update buffer ware + taken) buffer)]
        (loop [st (assoc state :buffer buffer)]
          (if (and (seq bill) (#(every?
                        (fn [[k req]] (>= (get % k 0) req))
                        bill) (:buffer st)))
            (let [{:keys [duration target-storage]} st]
              (Thread/sleep duration)

              (let [buf (reduce-kv
                          (fn [b k req] (update b k - req))
                          (:buffer st) bill)]

                (send (target-storage :worker) supply-msg amount)
                (recur (assoc st :buffer buf)))) st))))))
